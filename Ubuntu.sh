...
install_nginx_spdy() {
    ...
    [ -f /tmp/nginx-1.4.1.tar.gz ] || wget http://nginx.org/download/nginx-1.4.1.tar.gz -O /tmp/nginx-1.4.1.tar.gz
    ...
    [ -d /tmp/nginx-1.4.1 ] && cd /tmp/nginx-1.4.1 && make clean
    [ -d /tmp/openssl-1.0.1e ] && rm -rf /tmp/openssl-1.0.1e
    [ -d /tmp/nginx-1.4.1 ] || tar -zxvf /tmp/nginx-1.4.1.tar.gz -C /tmp
    [ -d /tmp/openssl-1.0.1e ] || tar -zxvf /tmp/openssl-1.0.1e.tar.gz -C /tmp
    # generate makefile
    cd /tmp/nginx-1.4.1 && ./configure \
...
install_gearmand() {
    ...
    [ -f /tmp/gearmand-1.1.8.tar.gz ] || wget https://launchpad.net/gearmand/1.2/1.1.8/+download/gearmand-1.1.8.tar.gz -O /tmp/gearmand-1.1.8.tar.gz
    [ -d /tmp/gearmand-1.1.8 ] && rm -rf /tmp/gearmand-1.1.8
...
server() {
    ...
    aptitude -y install php5 php5-xcache php5-cli php5-fpm php5-mysql php5-curl php5-geoip php5-gd php5-intl php5-mcrypt php5-memcache php-apc php5-imap php5-ming php5-ps php5-pspell php5-recode php5-snmp php5-sqlite php5-tidy php5-xmlrpc php5-xsl php5-cgi spawn-fcgi openssl geoip-database memcached
