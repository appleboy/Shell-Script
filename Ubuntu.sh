...
install_nginx_spdy() {
    ...
    [ -f /tmp/nginx-1.6.1.tar.gz ] || wget http://nginx.org/download/nginx-1.6.1.tar.gz -O /tmp/nginx-1.6.1.tar.gz
    ...
    [ -d /tmp/nginx-1.6.1 ] && cd /tmp/nginx-1.6.1 && make clean
    [ -d /tmp/openssl-1.0.1g ] && rm -rf /tmp/openssl-1.0.1g
    [ -d /tmp/nginx-1.6.1 ] || tar -zxvf /tmp/nginx-1.6.1.tar.gz -C /tmp
    [ -d /tmp/openssl-1.0.1g ] || tar -zxvf /tmp/openssl-1.0.1g.tar.gz -C /tmp
    # generate makefile
    cd /tmp/nginx-1.6.1 && ./configure \
    ...
    cd /tmp/nginx-1.6.1 && make && make install
}
